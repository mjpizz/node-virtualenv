...
function VirtualEnv(packagePath, options) {
  ...
  this._virtualenvSourceHome = path.join(path.dirname(packagePath), "node_modules", ".virtualenv-source");
...
VirtualEnv.prototype.install = function install(options, callback) {
  ...
  this._find(function(err, version, url) {
    ...
    // Bundle the create step so that it can be used regardless of whether
    // we re-download virtualenv or not.
    ...
    if (this._virtualenvSourceIsReady(version)) {
      continueFromCreate();
...
VirtualEnv.prototype._download = function _download(url, callback) {
  ...
  try {
    rimraf.sync(this._virtualenvSourceHome);
  } catch(err) {
    return callback(err);
  }
  ...
  var untarStream = tar.Extract({path: this._virtualenvSourceHome});
...
VirtualEnv.prototype._create = function _create(version, callback) {
  ...
  // Remove the old virtualenv if necessary.
  if (fs.existsSync(this._virtualenvHome)) {
    this._reportProgress("Recreating", this._virtualenvHome);
    try {
  ...
  virtualenvPath = this._getPathToSourceForVersion(version);
...
VirtualEnv.prototype._getPathToSourceForVersion = function _getPathToSourceForVersion(version) {
  return path.join(this._virtualenvSourceHome, "virtualenv-" + version)
}
